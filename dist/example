...
// Added popular brands as per BeautyHub reference
...
export default function HomePage({ onNavigate, user, onViewProduct, onSelectCategory, cartItemCount }) {
  ...
  const [searchQuery, setSearchQuery] = useState(""); // State for Search Engine
  ...
  const handleSearchSubmit = (e) => {
    if (e.key === 'Enter' && searchQuery.trim()) {
      onNavigate('products', { search: searchQuery }); // Sends search to products page
    }
  };
    ...
    <div className="min-h-screen bg-background">
      ...
      >
        <div className="container mx-auto px-4 py-3">
          <div className="flex items-center justify-between">
            ...
            <div className="hidden md:block max-w-md mx-8 flex-1">
              <div className="relative w-full">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground w-4 h-4" />
                <Input 
                  placeholder="Search for products..." 
                  className="pl-10 bg-input-background h-10 rounded-full border-none shadow-sm"
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  onKeyDown={handleSearchSubmit} // Search logic fix
                />
              </div>
            </div>
     ...
     {/* Shortened Hero Section */}
      <motion.section
        ...
        className="relative py-12 md:py-16 bg-gradient-to-br from-pink-50 to-orange-50"
      >
        <div className="container mx-auto px-4 grid md:grid-cols-2 gap-8 items-center">
          <motion.div initial={{ x: -50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: 0.6, delay: 0.2 }}>
            ...
            <p className="text-lg text-muted-foreground mb-6 max-w-md leading-relaxed">
              Premium skincare products curated for your beauty. Shop top brands with expert recommendations.
            </p>
            <div className="flex flex-col sm:flex-row gap-4">
              <Button
                size="lg"
                className="bg-slate-900 text-black hover:bg-slate-800 rounded-full px-10 h-14 font-bold shadow-lg"
          ...
          <motion.div initial={{ x: 50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: 0.6, delay: 0.4 }}>
            <ImageWithFallback
              ...
              className="w-full h-64 md:h-80 object-cover rounded-[3rem] shadow-2xl" // Hero Height reduced
            />
          </motion.div>
        </div>
      </motion.section>

      {/* NEW: Popular Brands Category */}
      <section className="py-12 bg-white">
        <div className="container mx-auto px-4">
          <h3 className="text-center font-black uppercase tracking-widest text-[10px] text-slate-400 mb-8">Popular Brands</h3>
          <div className="flex flex-wrap justify-center gap-8 md:gap-16 opacity-50 grayscale hover:grayscale-0 transition-all duration-700">
            {popularBrands.map(brand => (
              <span 
                key={brand} 
                className="text-lg md:text-2xl font-black text-slate-900 cursor-pointer hover:text-pink-600"
      ...
      <section className="py-20 bg-slate-50/50">
        <div className="container mx-auto px-4 text-center">
          ...
          <h3 className="text-4xl font-bold mb-12 tracking-tight text-slate-900">Our Glowing Customers</h3>
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {latestReviews.map((rev) => (
              <motion.div key={rev.id} className="p-8 bg-white rounded-[2.5rem] text-left border border-slate-100 shadow-sm" whileHover={{ y: -8, shadow: "0 10px 30px rgba(0,0,0,0.05)" }}>
