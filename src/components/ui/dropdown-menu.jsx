...
export function DropdownMenuCheckboxItem({ className, children, checked, ...props }) {
  return (
    <DropdownMenuPrimitive.CheckboxItem
      ...
      className={cn(
        "focus-accent focus-accent-foreground relative flex cursor-default items-center gap-2 rounded-sm py-1.5 pr-2 pl-8 text-sm outline-hidden select-none data-[disabled]-events-none data-[disabled]-50 [&_svg]-events-none [&_svg]-0 [&_svg([class*='size-'])]:size-4",
        className
      )}
      checked={checked}
      {...props}
...
export function DropdownMenuLabel({ className, inset, ...props }) {
  return (
    <DropdownMenuPrimitive.Label
      ...
      data-inset={inset}
      className={cn("px-2 py-1.5 text-sm font-medium data-[inset]-8", className)}
      {...props}
    />
  );
}
