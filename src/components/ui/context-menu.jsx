...
export function ContextMenuSubTrigger({ className, inset, children, ...props }) {
  return (
    <ContextMenuPrimitive.SubTrigger
      ...
      data-inset={inset}
      className={cn(
        "focus-accent focus-accent-foreground data-[state=open]-accent data-[state=open]-accent-foreground flex cursor-default items-center rounded-sm px-2 py-1.5 text-sm outline-hidden select-none data-[inset]-8 [&_svg]-events-none [&_svg]-0 [&_svg([class*='size-'])]:size-4",
        className
      )}
      {...props}
    >
...
export function ContextMenuCheckboxItem({ className, children, checked, ...props }) {
  return (
    <ContextMenuPrimitive.CheckboxItem
      ...
      className={cn(
        "focus-accent focus-accent-foreground relative flex cursor-default items-center gap-2 rounded-sm py-1.5 pr-2 pl-8 text-sm outline-hidden select-none",
        className
      )}
      checked={checked}
      {...props}
...
export function ContextMenuLabel({ className, inset, ...props }) {
  return (
    <ContextMenuPrimitive.Label
      ...
      data-inset={inset}
      className={cn("text-foreground px-2 py-1.5 text-sm font-medium", className)}
      {...props}
    />
  );
}
